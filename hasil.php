    ...
    <div class="card"> 
      ...
      <div class="card-body">
        ...
        <?php }elseif(count($komentarXLS)==0){ ?>
          <div class="alert alert-danger alert-dismissible fade show mb-0 text-center " style="padding-top: 15px;">
            <strong>ISI DATA PELABELAN ANDA KOSONG ATAU TIDAK DAPAT DIPROSES</strong>
          </div>
        <?php }elseif(count($komentarXLS)>0){ ?>
          <?php
            $BOWpositif=array();
...
<?php
$sentimen_hasil=array_count_values($sentimen_akhir);
if (isset($sentimen_hasil["Positif"])) {
  $hasil_positif=$sentimen_hasil["Positif"];
}else{
  $hasil_positif=0;
}
if (isset($sentimen_hasil["Negatif"])) {
  $hasil_negatif=$sentimen_hasil["Negatif"];
}else{
  $hasil_negatif=0;
} 
$dataPoints = array(array("label"=> "Positif", "y"=> $hasil_positif),array("label"=> "Negatif", "y"=> $hasil_negatif));
$txt="Analisis Sentimen terhadap ".count($komentarXLS)." data";
$txt2="Hasil Analisis Sentimen";
